...
function geocode_location_metadata(\Elgg\Event $event): void {
	...
	switch ($event->getName()) {
		case 'create':
		case 'update':
			$coordinates = elgg_trigger_event_results('geocode', 'location', ['location' => $metadata->value], false);
			if ($coordinates) {
...
function search_by_proximity_hook(\Elgg\Event $event): mixed {
	...
	$coords = elgg_trigger_event_results('geocode', 'location', ['location' => $query], false);
	if (!$coords) {
		return $return;
	}
